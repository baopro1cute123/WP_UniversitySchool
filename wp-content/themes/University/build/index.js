...
const liveSearch = new _modules_Search__WEBPACK_IMPORTED_MODULE_5__["default"]();
const likeBox = new _modules_Like__WEBPACK_IMPORTED_MODULE_2__["default"]();
...
class Like {
  ...
  event() {
    jquery__WEBPACK_IMPORTED_MODULE_0___default()(".like-box").on("click", e => this.dispatchLike(e));
  }
...
class Search {
  ...
  async getResults() {
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().get(universityData.root_url + "/wp-json/university/v1/universities?term=" + this.searchField.value); // gọi api
      const results = response.data; // lấy dữ liệu từ res => thay đổi nội dung html bên resultDiv
      this.resultsDiv.innerHTML = `
        <div class="row">
          <div class="one-third">
          ...
          <div class="one-third">
            ...
            ${results.programme.length ? '<ul class="link-list min-list">' : `<p>No programs match that search. <a href="${universityData.root_url}/programs">View all programs</a></p>`}
              ${results.programme.map(item => `<li><a href="${item.permalink}">${item.title}</a></li>`).join("")}
            ${results.programme.length ? "</ul>" : ""}
          ...
          <div class="one-third">
            ...
            ${results.event.length ? "" : `<p>No events match that search. <a href="${universityData.root_url}/event">View all events</a></p>`}
